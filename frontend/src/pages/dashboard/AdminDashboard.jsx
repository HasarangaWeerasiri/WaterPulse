...
export const AdminDashboard = () => {
  ...
  const refreshAuthorityWaterLogs = async () => {
    ...
    try {
      ...
      setAuthorityWaterLogsError(err?.response?.data?.message || 'Failed to load authority water logs');
  ...
  const handleDeleteWaterLog = async (logId) => {
    ...
    const confirmed = window.confirm('Delete this water log? This cannot be undone.');
    if (!confirmed) return;
    ...
    try {
      ...
      setDeleteWaterLogError(err?.response?.data?.message || 'Failed to delete water log');
    } finally {
  ...
  const groupWaterLogsByReport = useMemo(() => {
    ...
    for (const reportId of Object.keys(map)) {
      map[reportId].logs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    }

    // Sort report groups by the most recent status-changing log
    const groups = Object.entries(map).map(([reportId, group]) => {
      const logs = group.logs;
      const lastStatusChanging = logs.find((l) => l.safetyRating === 'Safe' || l.safetyRating === 'Unsafe') || logs[0] || null;
      return {
        reportId,
        report: group.report,
        logs,
    ...
    groups.sort((a, b) => new Date(b.lastStatusChanging?.createdAt || 0) - new Date(a.lastStatusChanging?.createdAt || 0));
    return groups;
  }, [filteredWaterLogs]);
  ...
  const getWhatHappened = (log) => {
    ...
    if (log.safetyRating === 'Unsafe') {
      const reasons = [];
      if (ph < 6.0 || ph > 9.0) reasons.push(`pH is ${ph} (outside safe range)`);
      if (turb > 10) reasons.push(`turbidity is ${turb} NTU (high)`);
      const reasonText = reasons.length ? reasons.join(' and ') : 'water parameters indicate unsafe conditions';
        ...
        {activeTab === 'waterlogs' && (
          <div className="p-6 bg-white shadow rounded-xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
              ...
              <div>
                <label className="block mb-2 text-sm font-semibold text-gray-700">Safety</label>
                <select
                  className="input"
              ...
              <div>
                <label className="block mb-2 text-sm font-semibold text-gray-700">Report Status</label>
                <select
                  className="input"
              ...
              <div>
                <label className="block mb-2 text-sm font-semibold text-gray-700">Creator Role</label>
                <select
                  className="input"
            ...
            {authorityWaterLogsLoading ? (
              ...
              <div className="p-4 text-gray-600 text-sm">No water logs found for the selected filters.</div>
            ) : (
                  ...
                  <div key={group.reportId} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-3">
                          <div className="text-xs text-gray-600">Current report status:</div>
                          <StatusBadge status={group.report?.status} />
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs text-gray-500">Water logs: {group.logs.length}</div>
                        {group.lastStatusChanging && (
                          <div className="text-xs text-gray-600 mt-1">
                            Latest status change: {group.lastStatusChanging.safetyRating}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="mt-4 space-y-3">
                      {group.logs.map((log) => {
                        ...
                        const isLastStatusChanging = group.lastStatusChanging && group.lastStatusChanging._id === log._id;

                        return (
                          <div
                            key={log._id}
                            className={`p-3 rounded-lg border bg-white ${
                              isLastStatusChanging ? 'border-blue-300 bg-blue-50' : 'border-gray-200'
                            }`}
                          >
                            <div className="flex items-start justify-between gap-4">
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-3">
                                  ...
                                  <div className="text-sm font-semibold text-gray-900 truncate">
                                    pH {log.phLevel}, turbidity {log.turbidity} NTU
                                  </div>
                                ...
                                <div className="text-xs text-gray-600 mt-1">
                                  ...
                                  <span className="font-semibold text-[#00569c]">
                                    {log.recordedBy?.firstName || 'Unknown'} {log.recordedBy?.lastName || ''}
                                  </span>
                                  {log.recordedBy?.email ? ` • ${log.recordedBy.email}` : ''}
                                </div>
                                <div className="text-xs text-gray-600 mt-1">
                              ...
                              <div className="text-right">
                                <div className="text-xs text-gray-500">Expected sync result</div>
                                <div className="text-xs font-semibold text-gray-900">{expectedStatus}</div>
                                ...
                                {user?.role === 'admin' && (
                                  ...
                                  >
                                    {deletingWaterLogId === log._id ? 'Deleting...' : 'Delete'}
                                  </button>
                                )}
                              </div>
                            </div>

                            <div className="text-sm text-gray-700 mt-2">{getWhatHappened(log)}</div>
